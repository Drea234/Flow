...
class DBManager:
    ...
    def _init_db(self):
        ...
        # Create conversations table with conversation_id and department fields
        cursor.execute('''
    ...
    def save_conversation(self, employee_id, employee_name, question, answer, summary=None, topic=None, conversation_id=None, department=None):
        """Save a conversation to the database with conversation grouping and department"""
        conn = self._get_connection()
        cursor = conn.cursor()
        ...
        # If department not provided, try to get it from employee data
        if not department:
            department = self.get_employee_department(employee_id)
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (employee_id, employee_name, question, answer, summary, topic, timestamp, conversation_id, department))
    ...
    def get_employee_department(self, employee_id):
        """Get department for an employee from the employee database"""
        try:
            with open("data/employee_database.json", "r") as f:
                employee_data = json.load(f)
            return employee_data.get(employee_id, {}).get("department", "Unknown")
        except:
            return "Unknown"
    ...
    def get_conversations_by_department(self, department, limit=100):
    ...
    def get_department_statistics(self):
        """Get conversation statistics grouped by department"""
        conn = self._get_connection()
        cursor = conn.cursor()
    ...
    def export_conversations_to_csv(self, file_path, filter_employee=None, filter_department=None):
        """Export conversations to CSV file with optional filters"""
        conn = self._get_connection()
    ...
    def get_conversation_stats(self):
        ...
        # Department count
        cursor.execute("SELECT COUNT(DISTINCT department) as department_count FROM conversations")
    ...
    def update_missing_departments(self):
        """Update conversations that are missing department information"""
        conn = self._get_connection()
        cursor = conn.cursor()
        
        # Get conversations with missing departments
        cursor.execute('''
        ...
        for conv in conversations:
            department = self.get_employee_department(conv['employee_id'])
            cursor.execute(
